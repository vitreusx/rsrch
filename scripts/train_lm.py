...
class TimeDelta(TypedDict):
    """A helper class for specifying time intervals (e.g. how often to
    validate or sample during the training process.)"""
...
class Config:
    ...
    min_bucket_char_count: int = 20480
    chars_per_batch: int = 2048
    # Model
    model_dim: int = 128  # 512
    num_blocks: int = 6
    num_heads: int = 8
    hidden_dim: int = 512  # 2048
    # Training process
    sample_size: int = 8
    sample_min_seq_len: int = 32
    sample_max_seq_len: int = 128
    val_every: TimeDelta = {"n": 2560, "of": "step"}
...
```"""
        ...
        dest = self.exp.dir / "samples" / f"step={self.step:07d}.md"
