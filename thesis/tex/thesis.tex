...
\begin{enumerate}
  ...
  \item $K_{\rm RL} = 8, N = 8, M = 1$: A variant of the first configuration, with more update epochs.
  \item $K_{\rm RL} = 16, N = 8, M = 1$: A more extreme version of the first preset, with an equal number of RL optimization steps, and greater data reuse.
  \item $K_{\rm RL} = 8, N = 4, M = 8$: We split the batch into $8$ minibatches, increasing the total number of parameter updates by $8 \times$.
\end{enumerate}

\paragraph{Results} If we take a look at the final validation scores (\autoref{fig:ppo_sweep}), we may infer that using PPO, in any of the tested configurations, has a positive effect on the performance. We may note that, except for the preset with $M = 8$, the training time is effectively the same.
...
Another method, which can be used to derive the value of $\alpha$ temperature, is auto-tuning, as described in (SAC Alg. and App. citation). Briefly, the idea is to specify a schedule of the mean policy entropy, rather than the $\alpha$ parameter itself, and optimize the value of $\alpha$ so that the schedule is achieved. There remains, however, a question of what kind of schedule to use. The recommendations for the model-free case suggest the use of a constant entropy target $0.89 H_{\rm max}$ for the discrete action spaces, where $H_{\rm max}$ denotes the maximum possible entropy for a policy in a given action space. However, the entropy curves of the best-performing agents in the baseline scenario (\autoref{fig:baseline_ent}) suggest, that we ought to specify a rapidly decaying entropy schedule, with the final entropy of approximately $0.05 H_{\rm max}$.
