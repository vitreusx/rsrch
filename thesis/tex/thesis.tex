...
\begin{enumerate}
  \item The input data for the optimization step may consist purely of \emph{imaginary} rollouts, wherein we sample a real state from a replay buffer, and use the model and the agent to simulate new rollouts in imagination. This approach forms the basis of the Dreamer series of architectures. In this case, the RL module may be \emph{any} model-free RL algorithm.
  \item A strategy developed in MBPO is to supplement on-policy real-environment data with simulated sequences. We are thus presented with a solution neither model-free, nor fully model-based.
  ...
  \item Planning methods may be augmented with neural networks, as was done in MuZero and EfficientZero.
\end{enumerate}
