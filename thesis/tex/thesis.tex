...
Let us start with the model validation loss. To be precise, we compute mean loss value over the batches of data drawn from the validation episodes. We will start with the aggregate results - the relationship between the final validation loss and the final agent performance. As we can see in \autoref{fig:model_val_loss}, it would appear, somewhat surprisingly, that the value is not meaningfully correlated with the final score achieved by the agent. A closer inspection of the training and validation loss curves (\autoref{fig:model_loss_curves}) reveals, that

\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth,height=0.8\paperheight,keepaspectratio]{assets/model_val_loss.pdf}
  \caption{Relationship between final validation score and final model validation loss. Each point represents a single training run. The x-axis represents the agent performance. The y-axis represents the value of the model validation loss at the end of the training.}
