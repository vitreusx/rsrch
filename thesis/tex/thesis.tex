...
\begin{figure}
  \centering
  \includegraphics[width=0.8\linewidth,height=0.8\paperheight,keepaspectratio]{assets/env_selection.pdf}
...
We begin with an experiment involving a straightforward increase of the frequency of optimization steps, without any further changes. The goals are as follows: to establish a baseline for further performance, to find the update-to-data regime where the agent performance degrades, and to investigate what happens at that point with the networks in question.
...
Beyond that, for testing purposes, we also add an online train-validation split mechanism: each episode in the replay buffer is assigned either to the training set or the validation set -- to be precise, every $(1/p)$-th episode is assigned to the validation set, where $p = 0.15$. Every $K$ optimization steps, a batch is sampled from the validation set, and validation loss and possibly other metrics are computed.
