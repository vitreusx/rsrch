...
Beyond that, we need to specify the training recipe. We leave the model update ratio fixed at $K_{\rm WM} = 4$, and select RL update ratio $K_{\rm RL}$ in such a way, that the clock time for a single training run is approximately the same, to ensure fair comparison. As the baseline, we select a scenario with $K_{\rm WM} = K_{\rm RL} = 4$.
