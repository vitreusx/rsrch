...
class Buffer(MutableMapping):
    ...
    def __getstate__(self):
        raise RuntimeError(
            ...
            "use of `DataLoader` (in which case the different worker processes"
    ...
    def step(self, seq_id: int, act, next_obs: dict):
        """Save an environment step, passing the action and the subsequent
        observation."""
...
class Observable(Wrapper):
    """An 'observable' version of the buffer. One can attach hooks to execute
    actions when sequences are added, updated or deleted."""
    ...
    def attach(self, hook: Hook, replay: bool = False):
        """Attach a hook. If `replay` is true, execute actions for each of
        the sequences in the buffer."""
