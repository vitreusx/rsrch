...
def gae_only_ret(
    ...
    :param reward: Tensor :math:`r_{1:L}` of shape (L, N) of rewards upon arriving at
        the state.
    :param next_value: Tensor :math:`r_{1:L}` of shape (L, N) of next-value estimates.
    :param next_gamma: Tensor :math:`\gamma_{1:L}` of shape (L, N) of :math:`\gamma` discounts for each state aside from the first one.
    """
