...
class OffPolicyRLLoader(IterableDataset):
    ...
    def __init__(
        ...
        :param sampler: An index sampler. Depending on `sampler_type`, it should either
            yield episode IDs, or tuples of episode IDs and slice positions.
...
class OnPolicyRLLoader(IterableDataset):
    ...
    def __init__(
        ...
        :param do_env_step: A function which returns next `(env_idx, (step, final))`
            tuple.
        :param temp_buf: A temp RL buffer used for storing and collating the
            env steps for the current batch.
        :param steps_per_batch: Number of env steps to return per single batch.
            In practice the batches may be smaller, if `min_seq_len` is greater
            than zero.
    ...
    def __iter__(self):
        ...
        while True:
            ...
            count = 0
            while count < self.steps_per_batch:
                env_idx, (step, final) = self.do_env_step()
                if env_idx in ep_ids:
                    count += 1
