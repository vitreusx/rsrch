...
class Beta1(Distribution, Tensorlike):
    def __init__(self, alpha: Tensor, beta: Tensor, event_dims: int = 0):
        shape = alpha.shape
        pivot = len(shape) - event_dims
        batch_shape, event_shape = shape[:pivot], shape[pivot:]
    ...
    def log_B(self):
        if self._log_B is None:
            t1 = torch.special.gammaln(1.0 + self.alpha)
            t2 = torch.special.gammaln(1.0 + self.beta)
            t3 = torch.special.gammaln(2.0 + self.alpha + self.beta)
            self._log_B = t1 + t2 - t3
        return self._log_B

    @property
    def mean(self):
        return (1.0 + self.alpha) / (2.0 + self.alpha + self.beta)
