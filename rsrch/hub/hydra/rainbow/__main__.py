...
def main():
    ...
    class Q(nn.Sequential):
        def __init__(self):
            enc = nets.NatureEncoder(env_f.obs_space.num_channels)
    ...
    def make_qf():
        ...
        if cfg.noisy.enabled:
            replace_with_noisy_(
                qf,
                sigma0=cfg.noisy.sigma0,
                factorized=cfg.noisy.factorized,
            )
        qf = qf.to(device)
        return qf
    ...
    while env_step < cfg.warmup:
        ...
        seq_ids[env_idx], slice_id = env_buf.push(seq_ids[env_idx], step)
        if slice_id is not None:
            new_slice_ids.append(slice_id)
    ...
    if isinstance(sampler, data.PrioritizedSampler):
        if len(new_slice_ids) > 0:
            sampler.update(np.array(new_slice_ids), 1.0e3)
    ...
    while env_step < cfg.total_steps:
        ...
        if slice_id is not None:
            ...
            sampler.update(slice_id, max_prio)
        ...
        while should_opt:
            ...
            with torch.no_grad():
                with autocast():
                    act = qf(batch.obs[-1]).argmax(-1)
                    target = qf_t(batch.obs[-1]).gather(-1, act[..., None]).squeeze(-1)
            ...
            if cfg.optimizer.grad_clip is not None:
                ...
                nn.utils.clip_grad_norm_(qf.parameters(), cfg.optimizer.grad_clip)
            scaler.step(qf_opt)
            scaler.update()
