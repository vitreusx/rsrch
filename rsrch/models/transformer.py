...
def scaled_dot_product_attention(
    ...
    Note: This impl is here mostly for "educational" purposes. Using `nn.functional.scaled_dot_product_attention` is better, since it uses FlashAttention.
    ...
    :param attn_mask: (Optional) A tensor of shape `(N, L, S)` denoting attention mask. Contributions to the attention from zeros in the `attn_mask` are zeroed.
    :param is_causal: (Optional) Whether the attention mask should be causal. This requires query and key seq lengths to be equal, and `attn_mask` to be `None`.
    ...
    if is_causal:
        ...
        l, s = qk.shape[-2:]
        attn_mask = torch.ones((l, s), device=qk.device)
