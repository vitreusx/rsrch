...
def split_freq_20k(test_name: str, suffix: str = ""):
    common_args = ["-p", "exp.split_freq", "grid_launch"]
    common_opts = {"_prefill": int(20e3)}
...
def main():
    ...
    p.add_argument("--test-name", default="split-freq-20k")
    p.add_argument("--sanity", action="store_true")
    args = p.parse_args()

    all_tests = split_freq_20k(args.test_name)
    if args.sanity:
