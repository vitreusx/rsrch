...
def main():
    ...
    def sync_hosts():
        ...
        try:
            ...
            logging.error(f"Sync on {host} was unsuccessful.")
            raise e
    ...
    def grid_launch():
        ...
        try:
            for host, cmds in zip(hosts, all_cmds):
                ...
                logging.info(shlex.join(CMD))
                remotes[host] = subprocess.check_output(CMD).decode().rstrip()

                host_cmd = tempfile.mktemp(prefix="cmd.")
                with open(host_cmd, "w") as f:
                    f.writelines(cmd + "\n" for cmd in cmds)

                CMD = ["scp", host_cmd, f"{host}:{remotes[host]}"]
                logging.info(shlex.join(CMD))
                subprocess.run(CMD, stdout=DEVNULL).check_returncode()
        ...
        for host in hosts:
            CMD = [
                ...
                args.session,
                "-d",
                f"parallel -j{args.tasks_per_host} -a {remotes[host]}",
            ]
            logging.info(shlex.join(CMD))
            subprocess.run(CMD).check_returncode()
